...
class OllamaClient:
    ...
    async def _save_state(self):
        previous_state = await self._load_latest_previous_state() or {}
        state_data = {
    ...
    async def _load_latest_previous_state(self) -> Optional[Dict]:
        ...
        for attempt in range(retries):
            if os.access(latest_file, os.R_OK):
                try:
                    ...
                    logger.error(f"Permission error when accessing file: {latest_file} - {e}")
                    if attempt < retries - 1:
                        time.sleep(2 ** attempt)  # Exponential backoff
                    else:
                        raise
                except Exception as e:
                    logger.error(f"Error when loading file: {latest_file} - {e}")
                    return None
            else:
                logger.error(f"File is not accessible for reading: {latest_file}")
                if attempt < retries - 1:
                    time.sleep(2 ** attempt)  # Exponential backoff
                else:
                    raise PermissionError(f"File is not accessible for reading: {latest_file}")
...
async def main():
    client = OllamaClient()
    document = await client.add_document("This is the Cognosis architecture.")
    if document:
