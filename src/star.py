...
class DependencyManager:
    ...
    def load(self, dep_name: str) -> bool:
        ...
        try:
            ...
            logging.warning(
                f"Dependency {dep_name} not found. Install via uv or add to /* script */."
            )
        return self.available[dep_name]
...
def bootstrap(script_path: str):
    ...
    try:
        ...
        print(
            "Error: 'uv' not installed. Run `pip install uv` or see https://astral.sh/uv."
        )
        sys.exit(1)
    ...
    if not IN_UV_ENV:
        ...
        if deps:
            ...
            try:
                ...
                print(
                    "Failed to install dependencies. Check your /* script */ comment."
                )
                sys.exit(1)
...
def start_lsp_server():
    ...
    if dep_mgr.load("python_lsp_server"):
        ...
        logging.warning(
            "LSP server disabled. Add python-lsp-server to /* script */ dependencies."
        )
...
class StarCore:
    ...
    def compute_rho(
        self, bits_erased: int, energy: float, cache_lines: int, delta_t: float
    ) -> float:
        try:
    ...
    def emit_null_vector(self, rank: int) -> Packet:
        ...
        bytecode = (
            array('B', [random.randint(0, 255) for _ in range(16)]).tobytes()
            + b'\x00' * 32
        )
    ...
    def ignition_check(self, rho: float) -> bool:
        ...
        d_rho_dt = (
            (rho - self.rho_history[-1]) / 1e-9 if len(self.rho_history) > 1 else 0.0
        )
    ...
    def absorb(self, absorber_id: int):
        ...
        while absorber.received:
            ...
            if abs(packet.rho - expected_rho) > 3 * statistics.stdev(
                self.rho_history or [RHO_0]
            ):
                print(
                    f"Absorber {absorber_id} desync! Rho mismatch: {packet.rho} vs {expected_rho}"
                )
                continue
            ...
            print(
                f"Absorber {absorber_id}: State {new_state}, Winding {new_winding}, Result {result}"
            )
