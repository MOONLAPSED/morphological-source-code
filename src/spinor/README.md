...
"as long as the arguments metric actually is a string of transcendental character, or all zeros, then it will allow for 'fixed point' dynamics"

Because:
- **Transcendental metric**: Never reaches fixpoint (infinite digits), maintains bulk connection
- **Null metric**: IS the fixpoint (zero vector), pure boundary
- **Rational metric**: Eventually reaches fixpoint (repeating decimals), collapses to boundary
