...
class SocketWrapper:
    ...
    def close(self) -> None:
        ...
        if not self._closed:
            try:
                ...
                logger.error(f"Error closing socket {self.id}: {e}")
            finally:
                self._closed = True
                logger.debug(f"Socket {self.id} closed")
    ...
    def get_peername(self) -> tuple:
        ...
        try:
            ...
            logger.error(f"Error getting peer name: {e}")
...
# -------------------- Enhanced Socket Wrapper --------------------

class SocketWrapper:
    """
    Enhanced socket wrapper with improved error handling and additional options.
    """
...
def enhanced_echo_handler(sock: SocketWrapper) -> Generator[Any, Any, None]:
    """
...
def main() -> None:
    ...
    try:
        server = ServerManager(config=config, handler=enhanced_echo_handler)
