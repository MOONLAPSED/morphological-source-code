...
class __Atom__(Generic[T, V, C], PyObjectLike):
    ...
    def __getattribute__(self, name: str) -> Any:
        print(f"__getattribute__ called with name: {name}")  # Debug statement
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if name == '_local_env' and not self._local_env:
            self._local_env = {}  # Initialize _local_env if it's being set for the first time
        print(f"__setattr__ called with name: {name}, value: {value}")  # Debug statement
