...
"""
# © 2025 Moonlapsed | CC BY | SEE LICENCE
import sys
import time
import mmap
import ctypes
...
class Config:
    ...
    def REGION_SIZE(self) -> int:
        ...
        return (self.PUBKEY_SIZE + self.MSG_SIZE + 
                self.MAC_SIZE + self.HASH_SIZE + self.FLAG_SIZE)
...
def worker_entrypoint(
    ...
    try:
        ...
        while int.from_bytes(
            bytes(peer_region[FLAG_OFFSET:FLAG_OFFSET+4]), 
            'little'
        ) < 1:  # Wait for READY
        ...
        if enable_oracle:
            ...
            if peer_status < 3:  # Peer not COMPLETE yet
                # We might be first!
                my_region[FLAG_OFFSET:FLAG_OFFSET+4] = (4).to_bytes(4, 'little')  # WINNER
            else:
                # Peer already complete
                my_region[FLAG_OFFSET:FLAG_OFFSET+4] = (3).to_bytes(4, 'little')  # COMPLETE
        else:
            my_region[FLAG_OFFSET:FLAG_OFFSET+4] = (3).to_bytes(4, 'little')  # COMPLETE
...
class QuantizedRuntime:
    ...
    def get_status(self) -> StatusFlag:
        ...
        flag_bytes = bytes(self.region[
            RegionLayout.FLAG_OFFSET:
            RegionLayout.FLAG_OFFSET + 4
        ])
        return StatusFlag(int.from_bytes(flag_bytes, 'little'))
    
    def get_public_key(self) -> bytes:
        """Read DH public key."""
        return bytes(self.region[
            RegionLayout.PUBKEY_OFFSET:
            RegionLayout.PUBKEY_OFFSET + self.config.PUBKEY_SIZE
        ])
    
    def get_message(self) -> bytes:
        """Read message payload."""
        return bytes(self.region[
            RegionLayout.MSG_OFFSET:
            RegionLayout.MSG_OFFSET + self.config.MSG_SIZE
        ]).rstrip(b'\x00')
    
    def get_mac(self) -> bytes:
        """Read HMAC."""
        return bytes(self.region[
            RegionLayout.MAC_OFFSET:
            RegionLayout.MAC_OFFSET + self.config.MAC_SIZE
        ])
    
    def get_code_hash(self) -> bytes:
        """Read quineic code verification hash."""
        return bytes(self.region[
            RegionLayout.HASH_OFFSET:
            RegionLayout.HASH_OFFSET + self.config.HASH_SIZE
        ])
...
def main():
    ...
    try:
        ...
        print(f"    Message: {runtime_a.get_message().decode('utf-8', errors='replace')}")
        print(f"    MAC: {runtime_a.get_mac().hex()[:32]}...")
        print(f"    Code Hash: {runtime_a.get_code_hash().hex()[:32]}...")
        
        print(f"\n  BETA Status: {status_b.name}")
        print(f"    Message: {runtime_b.get_message().decode('utf-8', errors='replace')}")
